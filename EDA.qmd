...
gdp_inflation_data <- world_bank_data %>%
...
```{r, echo=FALSE, fig.cap="", out.width="100%"}
ggplot(world_bank_data, aes(x = Year, y = FemaleLaborRate, color = Country)) +
  geom_line() +
  labs(
...
```{r, echo=FALSE, fig.cap="", out.width="100%"}
ggplot(world_bank_data, aes(x = Year, y = MaleLaborRate, color = Country)) +
  geom_line() +
  labs(
...
labor_unemployment_data <- world_bank_data %>%
  arrange(Country, Year) %>%
  group_by(Country) %>%
...
```{r}
ggplot(world_bank_data, aes(x = factor(Year), y =Inflation , size = GdpPerCapita)) +
  geom_point(aes(color = Country), alpha = 0.7) +
  labs(
    title = "Inflation vs GdpPerCapita by Year and Country",
    x = "Year",
    y = "Inflation",
    size = "GdpPerCapita",
    color = "Country"
